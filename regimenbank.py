...
def export_calendar_docx(reg: Regimen, start: dt.date, cycle_length: int, out_path: Path) -> bool:
    ...
    """
    Exports a week-grid calendar to a DOCX file with Name/DOB blanks.
    Returns True if successful, False if python-docx not installed.
    """
    try:
        from docx import Document
        from docx.shared import Pt, Inches
    ...
    # Header
    h = doc.add_paragraph()
    run = h.add_run(f"{reg.name} — Cycle 1")
    run.bold = True
    run.font.size = Pt(14)
    ...
    run = sub.add_run(f"{months} {title_year}")
    run.font.size = Pt(12)
    ...
    # Table: rows = number of weeks + header, cols = 7 (Sun-Sat)
    table = doc.add_table(rows=len(grid) + 1, cols=7)
    table.alignment = WD_TABLE_ALIGNMENT.CENTER
    table.autofit = True
    ...
    # Fill weeks
    for w_i, week in enumerate(grid, start=1):
        for d_i, cell in enumerate(week):
            c = table.cell(w_i, d_i)
            para = c.paragraphs[0]
            # Date line (bold)
            r1 = para.add_run(f"{calendar.month_abbr[cell['date'].month]} {cell['date'].day}\n")
            r1.bold = True
