...
function ConfirmDialog({ open, title, message, confirmLabel = "Delete", onConfirm, onClose }: { open: boolean; title: string; message: string; confirmLabel?: string; onConfirm: () => void; onClose: () => void; }) {
  return (
    <Dialog open={open} onClose={onClose} maxWidth="xs" fullWidth>
      <DialogTitle sx={{ fontWeight: 700 }}>{title}</DialogTitle>
      <DialogContent><Typography sx={{ fontSize: "0.9rem", color: "#475569" }}>{message}</Typography></DialogContent>
      <DialogActions sx={{ px: 3, pb: 2 }}><Button onClick={onClose} color="inherit">Cancel</Button><Button variant="contained" color="error" onClick={() => { onConfirm(); onClose(); }}>{confirmLabel}</Button></DialogActions>
    </Dialog>
  );
}
...
function RegimenEditor({ initial, onSaved, onDeleted, onShowHistory, isNew }: { initial: Regimen; onSaved: (name: string) => void; onDeleted: () => void; onShowHistory?: () => void; isNew: boolean; }) {
  ...
  function update<K extends keyof Regimen>(key: K, value: Regimen[K]) { setReg((p) => ({ ...p, [key]: value })); setDirty(true); }
  ...
  function handleTherapySave(t: Chemo) {
    setReg((p) => {
      ...
      if (therapyDialog.index === null) therapies.push(t); else therapies[therapyDialog.index] = t;
      return { ...p, therapies };
    });
    setDirty(true);
  }
          ...
          <Box sx={{ px: 2.5, py: 2, borderBottom: "1px solid #e2e8f0", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 2, flexWrap: "wrap" }}>
            <Box>
              <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                ...
                {!isNew && (
                  <Chip label={reg.on_study ? "On Study" : "Off Protocol"} size="small" sx={{ height: 20, fontSize: "0.68rem", fontWeight: 600, background: reg.on_study ? "#dbeafe" : "#f0fdf4", color: reg.on_study ? "#1d4ed8" : "#15803d" }} />
                )}
              </Box>
