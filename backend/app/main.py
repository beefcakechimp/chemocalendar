...
# ── lifespan: open pool + create tables on startup, close on shutdown ─────────

@asynccontextmanager
async def lifespan(app: FastAPI):
    init_db()          # opens pool + CREATE TABLE IF NOT EXISTS
    yield
    pool.close()
...
app.add_middleware(
    ...
    allow_origin_regex=(
        r"https://.*\.(railway\.app|up\.railway\.app|app\.github\.dev"
        r"|onrender\.com|vercel\.app)"
    ),
...
# ── bank (module-level, safe because lifespan opens pool before any request) ──
...
def rename_regimen(body: RenameRegimenRequest):
    ...
    if BANK.get_regimen(new):
        raise HTTPException(
            status_code=409, detail="A regimen with new_name already exists"
        )
...
def calendar_export(req: CalendarPreviewRequest):
    ...
    # /tmp is always writable (no persistent volume needed)
...
@app.get("/regimen-groups")
def get_regimen_groups() -> dict:
    return BANK.get_groups()


@app.post("/regimen-groups")
def save_regimen_groups(body: dict) -> dict:
    BANK.save_groups(body)
    return {"ok": True}
